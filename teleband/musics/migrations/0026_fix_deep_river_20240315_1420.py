...
def update_site_forward(apps, schema_editor):
    ...
    dr_bassline, dr_bassline_created = Part.objects.get_or_create(
        piece=deep_river, part_type=bassline
    )

    concert_tc, concert_tc_created = Transposition.objects.get_or_create(
        name="Concert Pitch TC"
    )
    concert_bc, concert_bc_created = Transposition.objects.get_or_create(
        name="Concert Pitch BC"
    )

    dr_melody_tc, dr_melody_tc_created = PartTransposition.objects.get_or_create(
        part=dr_melody, transposition=concert_tc
    )
    ...
    dr_bassline_bc, dr_bassline_bc_created = PartTransposition.objects.get_or_create(
        part=dr_bassline, transposition=concert_bc
    )
