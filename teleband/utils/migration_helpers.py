def create_part_et_al(apps, part, piece):
    ...
    for t in part["transpositions"]:
        ...
        part_trans = PartTransposition.objects.create(
            part=new_part, transposition=transposition, flatio=t["flatio"]
        )
...
def create_piece_et_al(apps, data):
    ...
    if not already_at_piece:
        piece = Piece.objects.create(
            name=data["name"],
            ensemble_type=EnsembleType.objects.get(name=data["ensemble_type"]),
        )
        if "video" in data:
